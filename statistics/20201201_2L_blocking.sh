echo "2020.12.1 " >> graphwalker_metrics.txt.statistics 
...
# logsize=32
# segsize=65536
# for((nverts_per_blk = 65536; nverts_per_blk <= 4194304; nverts_per_blk*=2))
# do
...
logsize=32
for((nverts_per_blk = 262144; nverts_per_blk <= 4194304; nverts_per_blk*=2))
do
    for((segsize = 32; segsize <= 256; segsize*=2))
    do
        echo "buffer capacity = 2MB, each block's #vertices = " $nverts_per_blk ", logsize = " $logsize ", segsize = " $segsize ", from echo" >> graphwalker_metrics.txt.statistics 
