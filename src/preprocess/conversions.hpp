...
#include "api/io.hpp"
     ...
     */
    static sid_t find_shards(std::string base_filename, long long shardsize) {
        std::string intervalfname = filename_intervals(base_filename, shardsize);
        FILE *invlf = fopen(intervalfname.c_str(),"r");
        if (!invlf) {
        ...
        fseek(invlf,0L,SEEK_END);  
        size_t sz = ftell(invlf);
        sid_t nshards = (sid_t)(sz/sizeof(vid_t)) - 1;
        fclose(invlf);
        return nshards;
    }
     ...
     * sharded or 0 if not found.
     */
    static sid_t find_shards(std::string base_filename, std::string shard_string="auto") {
        sid_t try_shard_num;
        sid_t start_num = 0;
        sid_t last_shard_num = 2400;
        ...
        for(try_shard_num=start_num; try_shard_num <= last_shard_num; try_shard_num++) {
            ...
            if (tryf >= 0) {
                // Found!
                close(tryf);
                sid_t nshards_candidate = try_shard_num;
                bool success = true;
    ...
    sid_t convert_by_shardsize(std::string filename, long long shardsize){

        long long malloc_size = shardsize * 1024 / sizeof(vid_t); //max number of edges of a shard
        ...
        *((eid_t*)beg_posptr) = cur_pos;
        ...
        while(fgets(s, 1024, inf) != NULL) {
            ...
            if( from == curvertex ){
                ...
                if( from - curvertex > 1 ){ //there are verts with zero out-links
                    ...
                    vid_t remainsize = malloc_size - ((csrptr-csr)/sizeof(vid_t));
        ...
        int f = open(intervalsFilename.c_str(), O_WRONLY | O_CREAT, S_IROTH | S_IWOTH | S_IWUSR | S_IRUSR);
        if (f < 0) {
            logstream(LOG_ERROR) << "Could not open " << intervalsFilename << " error: " << strerror(errno) << std::endl;
        }
        assert(f >= 0);
     ...
     */
    void convert_edgelist(std::string filename, sid_t nshards, vid_t nvertices, eid_t nedges) {
        unsigned long long malloc_size = (nvertices + nedges) / nshards + 1;
        logstream(LOG_INFO) << "malloc_size = " << malloc_size << std::endl;
        sid_t invlnum = convert_by_shardsize(filename, malloc_size);
        assert(invlnum==nshards);
    }

    sid_t convert_if_notexists(std::string basefilename, long long shardsize) {
        assert(shardsize > 0);
    ...
    sid_t convert_if_notexists_nshards(std::string basefilename, std::string nshards_string, vid_t nvertices, eid_t nedges, sid_t nshards) {
        if(nshards == 0 ){
            double max_shardsize = 1024. * 1024. * size_t(get_option_int("membudget_mb", 1024)) / 4 ;
            nshards = (sid_t) ( ((nedges+nvertices) * sizeof(VertexDataType) / max_shardsize) + 1.0);
