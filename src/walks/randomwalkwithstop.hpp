...
public:
    ...
    void updateByWalk(WalkDataType walk, unsigned walkid, unsigned exec_interval, eid_t *&beg_pos, vid_t *&csr, WalkManager &walk_manager ){ //, VertexDataType* vertex_value){
        unsigned threadid = omp_get_thread_num();
        WalkDataType nowWalk = walk;
        vid_t sourId = walk_manager.getSourceId(nowWalk);
        vid_t dstId = walk_manager.getCurrentId(nowWalk) + intervals[exec_interval].first;
        ...
        while (dstId >= intervals[exec_interval].first && dstId <= intervals[exec_interval].second && hop < maxwalklength ){
            // std::cout  << " -> " << dstId << " " << walk_manager.getSourceId(walk) << std::endl;
            updateInfo(sourId, dstId, threadid, hop);
            vid_t dstIdp = dstId - intervals[exec_interval].first;
            unsigned outd = beg_pos[dstIdp+1] - beg_pos[dstIdp];
            if (outd > 0 && (float)rand_r(&seed)/RAND_MAX > 0.15 ){
                unsigned pos = beg_pos[dstIdp] + ((unsigned)rand_r(&seed))%outd;
